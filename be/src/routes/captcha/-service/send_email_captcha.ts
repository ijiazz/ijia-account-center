...
import { CaptchaEmail, emailCaptchaService } from "./Email.service.ts";
import { createEmailCodeHtmlContent } from "./signup-email-code.ts";
import { APP_NAME, appConfig } from "@/config.ts";
import { select } from "@asla/yoursql";
import { v } from "@/sql/utils.ts";
import { EmailCaptchaActionType, EmailCaptchaQuestion } from "@ijia/account-dto";

export async function sendSignUpEmailCaptcha(email: string): Promise<EmailCaptchaQuestion> {
  const expire = 5 * 60; // 5 分钟有效期
  return sendEmailCaptcha(email, EmailCaptchaActionType.signup, expire, `HI! 您正在注册 ${APP_NAME}账号`);
}
