...
import { DbContext, test as viTest } from "./db_connect.ts";
import { createFetchSuite, HoFetch, HoFetchStatusError, InferFetchSuite } from "@asla/hofetch";
import { ApiDefined, REQUEST_AUTH_KEY } from "@ijia/account-dto";
  ...
  async hoFetch({ hono }, use) {
    const hoFetch = new HoFetch({
      fetch: async (url, option) => {
        const result = await hono.fetch(new Request(url, option));
