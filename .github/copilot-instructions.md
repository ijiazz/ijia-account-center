# IJIA School Account Center Copilot Instructions

本文件用于指导 GitHub Copilot 在本仓库中生成、修改和解释代码时遵循统一约定。

## Pull Request 相关

- Review PR 时，一律使用中文进行评论和沟通。

## 项目概况

- 语言主要为 TypeScript。
- 前端位于 `fe/`，使用 React、TanStack Router、Ant Design、Vite。
- 后端位于 `be/`，使用 Deno + Hono。
- 前后的接口共享类型与 DTO 位于 `dto/`。
...
- 前端代码放在 `fe/` 下，优先沿用现有 React 组件、路由和请求层组织方式。
...
- 后端代码放在 `be/src/` 下，接口实现优先遵循现有 Hono 路由组织结构。 或现有服务层直接重复实现。
